    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        def sum_food_proximity(cur_pos, food_positions, norm=False):
            ...
            for food in food_positions:
                food_distances.append(utils.manhattanDistance(food, cur_pos))
            if norm:
        ...
        def ghost_stuff(cur_pos, ghost_states, radius, scores):
            ...
            for ghost in ghost_states:
                if utils.manhattanDistance(ghost.getPosition(), cur_pos) <= radius:
                    scores -= 30
                    num_ghosts += 1
            return scores
...
def betterEvaluationFunction(currentGameState):
    ...
    def closest_ghost(cur_pos, ghosts):
        food_distances = []
        for food in ghosts:
            food_distances.append(utils.manhattanDistance(food.getPosition(), cur_pos))
        return min(food_distances) if len(food_distances) > 0 else 1
...
def contestEvaluationFunc(currentGameState):
    ...
    def closest_capsule(cur_pos, caps_pos):
        ...
        for caps in caps_pos:
            capsule_distances.append(utils.manhattanDistance(caps, cur_pos))
        return min(capsule_distances) if len(capsule_distances) > 0 else 9999999

    def scaredghosts(ghost_states, cur_pos, scores):
        scoreslist = []
        for ghost in ghost_states:
            if ghost.scaredTimer > 8 and utils.manhattanDistance(ghost.getPosition(), cur_pos) <= 4:
                scoreslist.append(scores + 50)
            if ghost.scaredTimer > 8 and utils.manhattanDistance(ghost.getPosition(), cur_pos) <= 3:
                scoreslist.append(scores + 60)
            if ghost.scaredTimer > 8 and utils.manhattanDistance(ghost.getPosition(), cur_pos) <= 2:
                scoreslist.append(scores + 70)
            if ghost.scaredTimer > 8 and utils.manhattanDistance(ghost.getPosition(), cur_pos) <= 1:
                scoreslist.append(scores + 90)
            #if ghost.scaredTimer > 0 and utils.manhattanDistance(ghost.getPosition(), cur_pos) < 1:
    ...
    def ghostattack(ghost_states, cur_pos, scores):
        ...
        for ghost in ghost_states:
            if ghost.scaredTimer == 0:
                scoreslist.append(scores - utils.manhattanDistance(ghost.getPosition(), cur_pos) - 10)
        return max(scoreslist) if len(scoreslist) > 0 else scores
